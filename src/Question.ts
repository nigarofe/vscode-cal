...
export class Question {
    ...
    constructor(dbData: any) {
        ...
        this.tags = dbData.tags ? dbData.tags.split(',').map((tag: string) => tag.trim()) : [];
        ...
        if (this.attempts.length === 0) {
            ...
            for (let j = 1; j < codeVec.length; j++) {
                if (codeVec[j] === 1) {
                    ...
                    const interval = Math.floor((curr.getTime() - prev.getTime()) / MS_PER_DAY);
            ...
            if (daysSinceLast === null) {
                ...
                this.potentialMemoryGainMultiplier = parseFloat((daysSinceLast / latestInterval).toFixed(2));
