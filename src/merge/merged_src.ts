...
export function getWebviewContent(
  ...
  md.renderer.rules.image = (tokens, idx, options, env, self) => {
    ...
    if (src && !src.startsWith("http") && docUri) {
      ...
      // Find the workspace folder that contains the current markdown file
      const workspaceFolder = vscode.workspace.getWorkspaceFolder(docUri);
      if (workspaceFolder) {
        console.log("banana debug 3");
        // Resolve the image path relative to the markdown file's folder
        const onDiskUri = vscode.Uri.joinPath(workspaceFolder.uri, src);

        // 👇 Add these logs to see the requested path
        console.log("--- Image Rendering ---");
        console.log("  Original src:", src);
        console.log("  Workspace Used:", workspaceFolder.uri.fsPath);
